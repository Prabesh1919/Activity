...
def main(def_args=sys.argv[1:]):
    ...
    if dates is not None:
        try:
            target_dates = [datetime.strptime(d.strip(), '%Y-%m-%d') for d in dates.split(',') if d.strip()]
        except ValueError:
            sys.exit('dates must be in YYYY-MM-DD format')
    elif dates_file is not None:
        try:
            with open(dates_file, 'r') as f:
                target_dates = [datetime.strptime(line.strip(), '%Y-%m-%d') for line in f if line.strip()]
        except ValueError:
            sys.exit('dates file must contain dates in YYYY-MM-DD format')
        except FileNotFoundError:
            sys.exit(f'dates file {dates_file} not found')
    else:
        target_dates = None
    ...
    if target_dates is not None:
        for day in target_dates:
            day = day.replace(hour=20, minute=0)
...
def arguments(argsval):
    ...
    parser.add_argument('-d', '--dates', type=str, required=False,
                        help="""A list of comma-separated dates (format YYYY-MM-DD) on which to generate commits.""")
    parser.add_argument('-df', '--dates_file', type=str, required=False,
                        help="""A path to a file containing a list of dates (one per line, format YYYY-MM-DD) on which to generate commits.""")
